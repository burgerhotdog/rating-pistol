...
const index_chance = (gameId, index) => {
  ...
  return 1 / GENERAL_LOOKUP[gameId].NUM_MAINSTATS;
};

export default (gameId, percentile, mainStatId, index) => {
  const times = 10000 / (10000 - percentile * 100);
  const spec_times = times / GENERAL_LOOKUP[gameId].STATS[mainStatId].mainChance[index];
  const runs = (spec_times / index_chance(gameId, index)) / (GENERAL_LOOKUP[gameId].DROPS_PER_RUN / 2);
  const days = (runs * GENERAL_LOOKUP[gameId].RESIN_PER_RUN) / GENERAL_LOOKUP[gameId].RESIN_PER_DAY;
  return days;
};
